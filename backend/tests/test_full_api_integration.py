...
async def test_full_api_flow(client: AsyncClient):
    ...
    if not food_cat:
        f_resp = await ac.post("/api/v1/categories", json={"name": "Food"})
        food_cat = f_resp.json()

    transport_cat = next((c for c in categories if c["name"] == "Transport"), None)
    if not transport_cat:
        t_resp = await ac.post("/api/v1/categories", json={"name": "Transport"})
    ...
    new_cat_resp = await ac.post(
        "/api/v1/categories", json={"name": f"Temp Category {uuid.uuid4().hex[:6]}"}
    )
